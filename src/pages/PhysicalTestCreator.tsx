...
export default function PhysicalTestCreator() {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [type, setType] = useState<'strength' | 'speed' | 'endurance' | 'agility' | 'power'>('strength');
  const [metrics, setMetrics] = useState<Option[]>([{ name: '', unit: '' }]);
          ...
          <CardContent className="py-8">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Only coaches and administrators can create physical tests.
              </p>
              <Button
                className="mt-4"
  ...
  const updateMetric = (index: number, field: keyof Option, value: string) => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Create Physical Test</h1>
